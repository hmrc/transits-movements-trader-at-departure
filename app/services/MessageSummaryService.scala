...
class MessageSummaryService {
  ...
  private[services] val declarationMessage: Reader[Departure, (Message, MessageId)] =
    Reader[Departure, (Message, MessageId)](_.messagesWithId match {
      ...
      case NonEmptyList((msg @ MessageWithStatus(_, _, _, DepartureDeclaration, _, _, _), id), tail) =>
        ...
        tail
          .foldLeft(NonEmptyList.of(head))({
            case (acc, (m @ MessageWithStatus(_, _, _, DepartureDeclaration, _, _, _), mid)) => acc :+ Tuple2(m, mid)
            case (acc, _)                                                                    => acc
          })
          .toList
  ...
  private val departureDeclarationCount: NonEmptyList[Message] => Int = {
    movementMessages =>
      movementMessages.toList.count {
        case MessageWithStatus(_, _, _, DepartureDeclaration, _, _, _) => true
        case _                                                         => false
      }
  }
    ...
    messagesWithId => messageType =>
      messagesWithId
        .foldLeft(Seq.empty[(MessageWithoutStatus, MessageId)]) {
          case (acc, (m @ MessageWithoutStatus(_, _, _, `messageType`, _, _), mid)) => acc :+ Tuple2(m, mid)
          case (acc, _)                                                             => acc
        }
  }
}
