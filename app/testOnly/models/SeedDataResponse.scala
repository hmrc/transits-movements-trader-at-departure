...
private[testOnly] case class SeedDataResponse(
  ...
  departureIdRangeStart: DepartureId,
  departureIdRangeEnd: DepartureId
)
...
private[testOnly] object SeedDataResponse {
  ...
  def apply(seedDataParameters: SeedDataParameters): SeedDataResponse = {
    ...
    val startDepartureId = seedDataParameters.startDepartureId

    val maxEori: SeedEori = SeedEori(startEori.prefix, startEori.suffix + numberOfUsers, startEori.padLength)

    val endDepartureId = DepartureId(startDepartureId.index + seedDataParameters.numberOfMovements - 1)
    ...
    SeedDataResponse(
      ...
      departureIdRangeStart = startDepartureId,
      departureIdRangeEnd = endDepartureId
    )
  }
